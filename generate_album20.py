#!/usr/bin/env python3
"""alma tamagotchi — album 20: 'dawn body'
   concept album: the arc of dawn in ~60 seconds total.
   each track explores a different stage of morning light,
   each ~15 seconds (2 patterns × 64 rows).
   the arc: pre-dawn stillness → first light → awakening → full dawn.
   major key, rising lines, ethereal textures.
"""
...
with open('album20_dawn_body.mod', 'wb') as f:
    f.write(output)

print(f"Album 'dawn body' written: {len(output)} bytes")
print(f"  Tracks: 4")
